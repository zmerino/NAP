...
% Import data table
filename = fullfile('data_2','meta_data_100.dat');
...
names = ["Trimodal-Normal","Uniform","Normal","Beta-a0p5-b1p5","Beta-a2-b0p5","Beta-a0p5-b0p5","Generalized-Pareto","Stable"];
labels = {'$2^{8}$', '$2^{9}$', '$2^{10}$', '$2^{11}$', '$2^{12}$',...
...
data_nap_parallel = data('NAP_{parallel}'==convertCharsToStrings(data.estimator),:);
data_nap_serial = data('NAP_{serial}'==convertCharsToStrings(data.estimator),:);
...
figure('Name',fig_name)
b = boxchart(log(data_nap_parallel.sample_power)/log(2), data_nap_parallel.cpu_time, 'GroupByColor',data_nap_parallel.distribution);
bp = gca;
bp.XAxis.TickLabelInterpreter = 'latex';
xlabel('$log_{2}(N)$','Interpreter','latex')
ylabel('CPU Time','Interpreter','latex')
...
figure('Name',fig_name)
b = boxchart(log(data_nap_serial.sample_power)/log(2), data_nap_serial.cpu_time, 'GroupByColor',data_nap_serial.distribution);
bp = gca;
bp.XAxis.TickLabelInterpreter = 'latex';
xlabel('$log_{2}(N)$','Interpreter','latex')
ylabel('CPU Time','Interpreter','latex')
