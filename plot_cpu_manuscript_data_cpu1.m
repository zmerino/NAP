...
filename = fullfile('data_many_cpu', 'meta_data_100.dat');
...
% fig_name = 'line_NAP_{serial} CPU per Distribution';
% figure('Name',fig_name)
% % compute average ---------------------------------------------------------
% data_nap_serial = convertvars(data_nap_serial,["distribution","name","estimator","sample_power"],"categorical");
% mean_data_serial = groupsummary(data_nap_serial,["distribution","name","estimator","sample_power"],"mean",["wall_time","cpu_time"]);
% T = mean_data_serial;
% % Group based on first column - gender column
% % Modify 1 to appropriate column number
% G = findgroups(T{:, 1});
