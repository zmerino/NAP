...
%         end
        m(1) = plot(actual.x,actual.pdf_y,'-k','DisplayName','$f(x)$');
        g(1) = plot(all_nmem{1,k}.x(:,1), all_nmem{1,k}.pdf(:,1),'-b','DisplayName','$\hat{f}(x)_{NMEM}$');
        h(1) = plot(all_se{1,k}.x(:,1), all_se{1,k}.pdf(:,1),'-r','DisplayName','$\hat{f}(x)_{SE}$');
        for i = 2:trialIndex
        plot(all_nmem{i,k}.x(:,1), all_nmem{i,k}.pdf(:,1),'-b')
        plot(all_se{i,k}.x(:,1), all_se{i,k}.pdf(:,1),'-r')
        end
%         m(1) = plot(actual.x,actual.pdf_y,'-k','DisplayName','$f(x)$');
        ...
        legend([h,g,m],'Location','NorthWest','Interpreter','latex')
        if save_graphics
