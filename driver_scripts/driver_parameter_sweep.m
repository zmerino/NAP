...
dim2 = 20;
max_bs = 1e5;
% max_bs = linspace(5e4, 1e5, dim1);
p1 = 1;
p2 = 0.55;
p3 = 1;
p4 = 0.33;
% p5 = 2;
% p5 = linspace(1, 2, dim1);
% p6 = linspace(0.0625, 0.625, dim2);
p5 = linspace(1, 5, dim1);
p6 = linspace(0.0625, 3, dim2);
p7 = 0.5;
p8 = 40;

% reshape vector based on hpyer paramter sweep
reshape_vec = [dim1, dim2];
hyper_params = combvec(p1,p2,p3,p4,p5,p6,p7,p8,max_bs);

mse_track = zeros(size(hyper_params, 2),1);
kl_track = zeros(size(hyper_params, 2),1);

format long

n_sweep = size(hyper_params,2);
for params_idx = 1:n_sweep
% for params = hyper_params
    params = hyper_params(:,params_idx);
    ...
    for j = 1:length(distribution_vector)
        ...
        for i = 1:trials
            ...
            for k = 1:length(sample_vec)
                ...
                % define current set of hyper parameters
                nse.p = params(1:end-1);
                nse.max_bs = params(end);
                disp(['%%%%%%%%%%%%%%%%%%: ',num2str(params_idx),'/',num2str(n_sweep)])
                disp(['Parameters: ',num2str(params')])
%                 disp(params')
...
%                 end
                ...
                disp([char(actual.dist_name),...
                    ...
                    num2str(fail_nmem(k,i,j)), newline])
                ...
                kl_track(params_idx) = kl_se_test;
                mse_track(params_idx) = mse_se_test;
...
kl_reshape = reshape(kl_track, reshape_vec);
mse_reshape = reshape(mse_track, reshape_vec);

figure('Name', '2D Image: KL')
imagesc(p5,p6,kl_reshape)
colorbar

figure('Name', '2D Image2: MSE')
imagesc(p5,p6,mse_reshape)
colorbar
