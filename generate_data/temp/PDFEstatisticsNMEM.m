...
%             all_nmem{i,k}.sqr(:,1) = NMEM_sqr;
    ...
    for k = 1:length(sample_vec)
        ...
        if sum(passMat(:,:,1),1) > 0
            h(1) = plot(all_nmem{1,k}.x(:,1), all_nmem{1,k}.pdf(:,1),'-','DisplayName','$\hat{f}^{(k)}(x)_{SE}$');
            for i = 1:trialIndex
...
%             plot(xHTse,yHTse,'.')
        ...
        graphics_name = ['heavyTail_NMEM_D_',...
            char(actual.dist_name),...
            'T_',int2str(trials),...
            'S_',int2str(sample_vec(k)),...
            '_',int2str(percSample),...
...
%             for i = 1:length(sum(passMat(:,:,1),1)>0)
        for i = 1:trials
            if ~isempty(all_nmem{i,k})
                plot(all_nmem{i,k}.x(:,1), all_nmem{i,k}.pdf(:,1),'-','DisplayName','$\hat{f}^{(k)}(x)_{SE}$');
            else
                continue;
            end
%             pause
        end
        h(1) = plot(actual.x,actual.pdf_y,'--k','DisplayName','$f(x)$');
        ...
        if ~isempty(all_nmem{i,k}.x(:,1))
            h(1) = plot(all_nmem{i,k}.u(:,1), all_nmem{i,k}.sqr(:,1),'-','DisplayName','$\hat{f}^{(k)}(x)_{SE}$');
        else
            continue;
        end
...
%     end
%
%     figure('Name','NMEMpdf JS Box PLot')
...
%     hold on
%     plot(sample_vec,fail_rate_nmem/trials,'-k','DisplayName',char(distribution_vector(j)))
%     xlabel('N','Interpreter','latex')
%     ylabel('Failure Rate','Interpreter','latex')
%     ylim([0,1.125])
%     if save_graphics
%         graphics_name = ['failed_Estimates_NMEMpdf_',...
%             char(actual.dist_name),...
%             'T_',int2str(trials)];
%         png_file = strcat(graphics_name,'.png');
%         saveas(gcf,png_file)
...
for ii = 1:length(distribution_vector)
    ...
    h(ii) = plot(log(cpu.stdTimeNMEMpdf{ii}(:,1))/log(2), cpu.stdTimeNMEMpdf{ii}(:,2),strcat('-',Markers{ii}),'Color',cc(ii,:),...
        'MarkerEdgeColor',cc(ii,:),...
        'MarkerFaceColor',cc(ii,:),...
        'MarkerSize',5',...
        'DisplayName',char(dist_plot_name(ii)));
