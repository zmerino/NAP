...
%             figure('Name',['br per level.',' Size: ',num2str(Ns), '-', char(filename)])
%             hold on
...
%             xlabel('Tree Level')
%             legend('Threshold')
%             graphics_name = ['brPerLevel-', char(filename)];
...
%             print(fig_file,'-depsc')
            ...
            figure('Name',['br per level.',' Size: ',num2str(Ns), '-', char(filename)])
            hold on
            plot(0:size(plevel,1),T*ones(size(plevel,1)+1), '-r');
            for k = 1:size(BRlevel,1)
                plot((k-1)*ones(size(BRlevel{k,1}(1,:),1),1),...
                    BRlevel{k,1}(1,:),...
                    'o',...
            ...
            publicationQualityHalfSize();
            
            %--------------------------------------------------------------------------
            figure('Name',['tree branching hist.',' Size: ',num2str(Ns), '-', char(filename)])
            histogram(sample)
            ylabel('Data Points per Bin','Interpreter','latex')
            graphics_name = ['treeBranchingHist-', char(filename)];
            png_file = strcat(graphics_name,'.png');
            saveas(gcf,png_file)
            fig_file = strcat(graphics_name,'.eps');
            print(fig_file,'-depsc')
            
            figure('Name',['tree branching struct.',' Size: ',num2str(Ns), '-', char(filename)])
            hold on
            ...
            graphics_name = ['treeBranchingStruct-', char(filename)];
            png_file = strcat(graphics_name,'.png');
            saveas(gcf,png_file)
            fig_file = strcat(graphics_name,'.eps');
            print(fig_file,'-depsc')
...
%             r = dxMin/dxMax;
        ...
        function [dxbr,brL,brR,partition] = minimizeBRdiff(sample,window,p,binMin,filename,j)
            ...
            % % % % % % % sample1 = -3 + randn(500,1);
            % % % % % % % sample2 = 10 + randn(500,1);
            % % % % % % % sample = [sample1 ; sample2];
            % % % % % % binMin = 11;
            % % % % % % window = 10;
            % % % % % % p = [1,1,0];
            % % % % % % sample = importdata('D_Normal/D_Normal_T_1_S_1024.txt');
            % % % % % % sample = importdata('D_Beta-a0p5-b0p5/D_Beta-a0p5-b0p5_T_1_S_1024.txt');
            % % % % % % Ns = length(sample);
            % function definition
%             br = @brProduct;
            ...
            if j == 1
                ...
                figure('Name','BRleft and BRright')
%                 subplot(2,2,[1,2])
                hold on
                plot(1:length(BRright),BRright,'--r')
                plot(1:length(BRleft),BRleft,'--b')
                plot(1:length(dxbrTrack),dxbrTrack,'-k')
                ylabel('$\Delta \xi_0 = \xi_0 - \xi_1$','Interpreter','latex')
                xlabel('$n_{partition}$','Interpreter','latex')
%                 xlim([0,Ns])
                legend('$\xi_0$','$\xi_1$','Interpreter','latex')
                graphics_name = ['BRAcrossSample_',char(filename)];
                png_file = strcat(graphics_name,'.png');
                saveas(gcf,png_file)
                fig_file = strcat(graphics_name,'.eps');
                print(fig_file,'-depsc')
                
                publicationQualityQuarterSize();
                
                figure('Name','Rleft and Rright')
%                 subplot(2,2,3)
                hold on
                plot(1:length(rLTrack),rLTrack,'-b')
                plot(1:length(rRTrack),rRTrack,'-r')
                ylabel('$R$','Interpreter','latex')
                xlabel('$n_{partition}$','Interpreter','latex')
%                 xlim([0,Ns])
                graphics_name = ['RAcrossSample_',char(filename)];
                png_file = strcat(graphics_name,'.png');
                saveas(gcf,png_file)
                fig_file = strcat(graphics_name,'.eps');
                print(fig_file,'-depsc')
                ...
                figure('Name','Bleft and Bright')
%                 subplot(2,2,4)
                hold on
                plot(1:length(bLTrack),bLTrack,'-b')
                plot(1:length(bRTrack),bRTrack,'-r')
                ylabel('$B$','Interpreter','latex')
                xlabel('$n_{partition}$','Interpreter','latex')
%                 xlim([0,Ns])
                graphics_name = ['BAcrossSample_',char(filename)];
                png_file = strcat(graphics_name,'.png');
                saveas(gcf,png_file)
                fig_file = strcat(graphics_name,'.eps');
                print(fig_file,'-depsc')
                
               figure('Name','dx left and right')
%                 subplot(2,2,4)
                hold on
                plot(1:length(dxLMinTrack),dxLMinTrack,'-b')
                plot(1:length(dxLMaxTrack),dxLMaxTrack,'--b')
                plot(1:length(dxRMinTrack),dxRMinTrack,'-r')
                plot(1:length(dxRMaxTrack),dxRMaxTrack,'--r')
                ylabel('$B$','Interpreter','latex')
                xlabel('$n_{partition}$','Interpreter','latex')
%                 xlim([0,Ns])
                graphics_name = ['DXcrossSample_',char(filename)];
                png_file = strcat(graphics_name,'.png');
                saveas(gcf,png_file)
                fig_file = strcat(graphics_name,'.eps');
                print(fig_file,'-depsc')
...
%                 figure('Name','T test 1')
% %                 subplot(2,2,4)
%                 hold on
%                 plot(1:length(bRTrack),bRTrack.*(1-rRTrack),'-r')
%                 plot(1:length(bLTrack),bLTrack.*(1-rLTrack),'-b')
%                 ylabel('$B$','Interpreter','latex')
%                 xlabel('$n_{partition}$','Interpreter','latex')
%                 xlim([0,Ns])
%                 graphics_name = ['BAcrossSample_',char(filename)];
...
%                 print(fig_file,'-depsc')
%                 
%                  figure('Name','T test 2')
% %                 subplot(2,2,4)
%                 hold on
%                 plot(1:length(bRTrack),(1-rRTrack),'-r')
%                 plot(1:length(bLTrack),(1-rLTrack),'-b')
%                 ylabel('$B$','Interpreter','latex')
%                 xlabel('$n_{partition}$','Interpreter','latex')
%                 xlim([0,Ns])
%                 graphics_name = ['BAcrossSample_',char(filename)];
...
% %                 subplot(2,2,4)
%                 hold on
%                 plot(1:length(bRTrack),bRTrack./(rRTrack),'-r')
%                 plot(1:length(bLTrack),bLTrack./(rLTrack),'-b')
%                 ylabel('$B$','Interpreter','latex')
%                 xlabel('$n_{partition}$','Interpreter','latex')
%                 xlim([0,Ns])
%                 graphics_name = ['BAcrossSample_',char(filename)];
