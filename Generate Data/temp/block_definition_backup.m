classdef block_definition
    methods(Static)
            ...
            if BR0 > T
                for jj = 1:maxLevel
                    ...
                    for b = 1:nbranch
                        ...
                        [dxBR,brL,brR,partition] = block_definition.min_br_gold(sample(pList(b):pList(b+1)),window,p,binMin);
        ...
        function [dxbr1,brL,brR,partition] = min_br_gold(sample,window,p,binMin)
            ...
            %plot BR of partions ------------------------------------------
                ...
                subplot(2,2,[1,2])
                hold on
                plot(1:length(BRright),BRright,'--','Color',[0.5,0.5,0.5])
                plot(1:length(BRleft),BRleft,'--k')
                ...
                subplot(2,2,3)
                hold on
                plot(1:length(rLTrack),rLTrack,'-k')
                plot(1:length(rRTrack),rRTrack,'Color',[0.5,0.5,0.5])
                ...
                subplot(2,2,4)
                hold on
                plot(1:length(bRTrack),bRTrack,'Color',[0.5,0.5,0.5])
                plot(1:length(bLTrack),bLTrack,'-k')
                ylabel('$B$','Interpreter','latex')
                xlabel('$n_{partition}$','Interpreter','latex')
                xlim([0,Ns])
                
                graphics_name = [num2str(Ns),'_BRacrossSample_',char(filename)];
                png_file = strcat(graphics_name,'.png');
                saveas(gcf,png_file)
                fig_file = strcat(graphics_name,'.eps');
                print(fig_file,'-depsc')
