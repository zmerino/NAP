    ...
    for k = 1:length(sample_vec)
        ...
        for i = 1:trialIndex
            if ~isempty(all_nmem{i,k}.x(:,1))
                h(1) = plot(all_nmem{i,k}.x(:,1), all_nmem{i,k}.pdf(:,1),'-','DisplayName','$\hat{f}^{(k)}(x)_{SE}$');
            else
                continue;
            end
        end
...
%             plot(xHTse,yHTse,'.')
    ...
    boxplot(MSE_nmem,sample_data_box,'Labels',labels2)
    bp = gca;
    ...
    boxplot(kl_nmem,sample_data_box,'Labels',labels2)
    bp = gca;
    bp.XAxis.TickLabelInterpreter = 'latex';
