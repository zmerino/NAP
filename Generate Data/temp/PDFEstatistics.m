...
labels = {'$2^{9}$','$2^{10}$','$2^{11}$','$2^{12}$','$2^{13}$','$2^{14}$',...
...
%             js_nmem = [js_nmem,JSDiv(kl_dist.pdf_y', pdf_NMEM')];
        ...
        disp(['i: ',num2str(i),'j: ',num2str(i),'k: ',num2str(i)])
        disp(SEtripFlag)
        disp(NMEMtripFlag)
...
%         end
    ...
    for k = 1:length(sample_vec)
        ...
        hold on
        if SEpdfpass(k,1) ~= 0 & NMEMpdfpass(k,1) ~= 0
            disp(['SEpdfpass: ', num2str(SEpdfpass(k,1))])
            disp(['k: ', num2str(k)])
            disp('t1')
        ...
        for i = 2:trialIndex
            if SEpdfpass(k,i) ~= 0 & NMEMpdfpass(k,i) ~= 0
