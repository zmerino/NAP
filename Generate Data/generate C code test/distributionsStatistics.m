...
% xticklabels(labels)
ylabel('$mean(\xi)$','Interpreter','latex')
legend([h])
if save_graphics
...
for j = 1:length(distribution_vector)
    ...
    hold on
%     plot(fail_rate_nmem(:,1),fail_rate_nmem(:,2)/trials,'-k','DisplayName',char(distribution_vector(j)))
    g(1) = plot(fail_rate_nmem(:,1),fail_rate_nmem(:,2)/trials,'-r','DisplayName','$NMEM$');
    g(2) = plot(fail_rate_se(:,1),fail_rate_se(:,2)/trials,'-k','DisplayName','$SE$');
