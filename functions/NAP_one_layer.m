...
%         p8 = 40;
    ...
    methods
        function [obj] = stitch(obj, inputSample, serial)
            ...
            obj.nBlocks = length(kList)+1;
%             kBlockLower = horzcat([[1],kList(1:end-1)]);
%             kBlockUpper = horzcat([kList(2:end),[obj.N]]);
            kBlockLower = [1,kList(1:end-1)'];
            kBlockUpper = [kList(2:end)',obj.N];
...
%             for b=1:obj.nBlocks
            blockScale = zeros(1,obj.nBlocks-1);     % => length of x as the span of a block
            blockShift = zeros(1,obj.nBlocks-1);     % shift in x to center the span about 0
            for b=1:obj.nBlocks-1
                kL = kBlockLower(b);
                kU = kBlockUpper(b);
...
%             title('Data Points per Block','Interpreter','latex')
            ...
            nBs0 = blockSize - 1;
            nBs0(1) = nBs0(1) + 0.5;
            nBs0(obj.nBlocks-1) = nBs0(obj.nBlocks-1) + 0.5;
...
%                 sample = s*( x(j1+1:j2-1) - blockShift(b) )';       % Note: x is sorted
                sample = s*( x(j1:j2) - blockShift(b) )';       % Note: x is sorted
                ...
                tempStruc.SURDtarget = targetCoverage;
                if obj.mod_estimate
...
%                 tempStruc.scoreType = 'QZ';                % {'QZ', 'LL'}
            ...
            parfor (b=1:obj.nBlocks-1, parforArg)
...
%             while( b <= obj.nBlocks )
            ...
            for b=1:length(indexList)-1
                ...
                if( k0 < 1 ) %51 <------------------------------ currently very small!
                    ...
                    hold on;
                    for nb=1:length(indexList)-1
                        block_dat = targetBlock{1,indexList(nb)}.data(:,1);
