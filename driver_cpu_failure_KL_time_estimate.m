...
status = mkdir('log');
filename = ['Meta_Data_Total_Time-', datestr(datetime(floor(now), 'ConvertFrom', 'datenum')), '.txt'];
full_file = fullfile('log', filename);

fid = fopen(full_file, 'w');
fprintf(fid,['Time estimate for meta data run started on: ', datestr(datetime(now, 'ConvertFrom','datenum')), '\n']);
...
writetable(global_table,full_file('data','estimator_meta_data.dat'))
