...
for j = 1:length(distribution_vector)
    ...
    kl_vec_se_parallel = zeros(length(sample_vec),trials);
    kl_vec_se_serial = zeros(length(sample_vec),trials);
    ...
    for k = 1:length(sample_vec)
        ...
        for i = 1:trials
            ...
            actual.min_limit = min(SE_x);
            actual.max_limit = max(SE_x);
            actual.x = SE_x;
            actual = actual.dist_list();

            kl_vec_se_parallel(k,i) = utils_analysis.mse(actual.pdf_y, SE_pdf);


            % get actual distribution
            actual.min_limit = min(x_NMEM);
            actual.max_limit = max(x_NMEM);
            actual.x = x_NMEM;
            actual = actual.dist_list();

            kl_vec_nmem(k,i) = utils_analysis.mse(actual.pdf_y, pdf_NMEM);
